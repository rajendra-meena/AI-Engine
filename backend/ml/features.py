...
class FeatureEngine:
    ...
    @staticmethod
    def compute_returns(closes: list[float], periods: list[int] = [1, 5, 10, 20]) -> dict[str, float]:
        result = {}
        for p in periods:
            if len(closes) > p:
                result[f"return_{p}"] = (closes[-1] - closes[-p - 1]) / closes[-p - 1] * 100
    ...
    def compute_macd(closes: list[float]) -> dict[str, float]:
        ...
        return {"macd": macd_line, "macd_signal": macd_line * 0.9, "macd_hist": macd_line * 0.1}
    ...
    @staticmethod
    def compute_supertrend(closes: list[float], highs: list[float], lows: list[float], period: int = 10, multiplier: float = 3.0) -> dict[str, float]:
        atr = FeatureEngine.compute_atr(highs, lows, closes, period)
        hl2 = (highs[-1] + lows[-1]) / 2 if highs else closes[-1]
        upper = hl2 + multiplier * atr
        lower = hl2 - multiplier * atr
        return {"supertrend_upper": upper, "supertrend_lower": lower, "supertrend_mid": (upper + lower) / 2}

    @staticmethod
    def compute_bollinger(closes: list[float], period: int = 20, multiplier: float = 2.0) -> dict[str, float]:
        if len(closes) < period:
            return {"bb_upper": closes[-1], "bb_mid": closes[-1], "bb_lower": closes[-1]}
        sma = sum(closes[-period:]) / period
        variance = sum((c - sma) ** 2 for c in closes[-period:]) / period
        std = math.sqrt(variance)
        return {"bb_upper": sma + multiplier * std, "bb_mid": sma, "bb_lower": sma - multiplier * std}

    @staticmethod
    def compute_all(highs: list[float], lows: list[float], closes: list[float]) -> dict[str, float]:
        features = {}
