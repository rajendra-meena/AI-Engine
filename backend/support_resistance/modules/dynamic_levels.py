    ...
    def generate(indicator_snap: dict[str, Any] | None) -> list[SRLevel]:
        ...
        for key, label in emas:
            ...
            if val and close:
                ...
                levels.append(SRLevel(price=round(val, 2), level_type=type_str,
                                      source=key, strength="NORMAL", label=label))

        vwap = indicator_snap.get("vwap")
        if vwap and close:
            type_str = "support" if close > vwap else "resistance"
            levels.append(SRLevel(price=round(vwap, 2), level_type=type_str,
                                  source="vwap", strength="STRONG", label="VWAP", is_major=True))
        ...
        if st_upper and st_trend == "DOWN":
            levels.append(SRLevel(price=round(st_upper, 2), level_type="resistance",
                                  source="supertrend", strength="STRONG", label="SuperTrend R", is_major=True))
        if st_lower and st_trend == "UP":
            levels.append(SRLevel(price=round(st_lower, 2), level_type="support",
                                  source="supertrend", strength="STRONG", label="SuperTrend S", is_major=True))
