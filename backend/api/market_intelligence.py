...
async def economic_calendar(country: str = "", from_date: str = "", to_date: str = ""):
    events = []
    names = ["Interest Rate Decision", "CPI Data", "GDP Growth", "Employment Change", "FOMC Minutes", "RBI Repo Rate", "Industrial Production", "Retail Sales"]
...
async def news_intelligence(symbol: str = "", category: str = "", limit: int = 20):
    ...
    sources = ["Bloomberg", "Reuters", "CNBC", "Economic Times", "Mint", "Business Standard"]
    for i in range(min(limit, 15)):
        items.append({
            ...
            "publishedAt": (datetime.utcnow() - timedelta(hours=random.randint(0, 24))).isoformat(),
        })
    return items
...
async def options_intelligence(symbol: str = "NIFTY 50"):
    return {
        ...
        "oiBuildUp": [{"strike": s, "type": random.choice(["ce", "pe"]), "oi": int(_random_float(100000, 500000)), "change": int(_random_float(-10000, 10000))} for s in range(19000, 20100, 100)],
...
async def sector_intelligence():
    sectors = ["IT", "Banking", "Pharma", "Auto", "FMCG", "Energy", "Metal", "Realty", "PSU", "Media"]
    return [{"name": s, "change": _random_float(-3, 3), "relativeStrength": _random_float(0.5, 1.5), "momentum": _random_float(-2, 2), "trend": random.choice(["uptrend", "downtrend", "ranging"]), "leadership": _random_float(0, 100), "capitalRotation": _random_float(-50, 50)} for s in sectors]

@router.get("/api/intelligence/regime")
async def market_regime():
    return {
        "regime": random.choice(["GROWTH", "RISK_ON", "RISK_OFF", "DEFENSIVE", "RECOVERY"]),
