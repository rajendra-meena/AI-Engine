...
def _compute_metrics(trades: list[dict]) -> dict:
    ...
    return {
        "totalTrades": total, "wins": len(wins), "losses": len(losses),
        "winRate": wr, "netProfit": round(net, 2), "grossProfit": round(gp, 2),
        "grossLoss": round(gl, 2), "profitFactor": round(pf, 2),
...
async def run_backtest(body: BacktestRequest):
    ...
    metrics["equityCurve"] = [{"date": f"2026-01-{i+1:02d}", "value": 100000 + sum(t.get("pnl", 0) for t in trades[:i+1])} for i in range(0, len(trades), max(1, len(trades)//30))]
    return metrics
...
async def run_walkforward(body: WalkForwardRequest):
    ...
    for i in range(num_windows):
        trades = [{"pnl": random.gauss(0, 800) * (1 if random.random() > 0.4 else -1)} for _ in range(random.randint(10, 30))]
        oos_results.append(_compute_metrics(trades))

    combined = oos_results[-1] if oos_results else {}
    return {"inSample": oos_results[0] if oos_results else {}, "outOfSample": oos_results[-1] if oos_results else {}, "combined": combined, "windows": windows}
...
async def run_montecarlo(body: MonteCarloRequest):
    ...
    return {
        "simulations": sims, "meanReturn": round(mean, 2), "medianReturn": round(median, 2),
        "stdReturn": round(std, 2), "var95": round(results[int(sims * 0.05)], 2),
        "var99": round(results[int(sims * 0.01)], 2),
        "maxReturn": round(results[-1], 2), "minReturn": round(results[0], 2),
        "percentPositive": round(pos / sims * 100, 2),
        "distribution": [{"range": f"{int(results[0])}-{int(results[-1])}", "count": sims}],
    }
...
async def run_optimization(body: OptimizationRequest):
    ...
    for i in range(10):
        results.append({
            "id": f"opt_{i}",
            "params": {p["key"]: p.get("min", 0) + i * p.get("step", 1) for p in body.params},
            "metrics": {
...
async def optimize_portfolio(body: PortfolioRequest):
    ...
    return {
        "allocations": [{"strategyId": s.get("id", ""), "name": s.get("name", ""), "weight": equal_w} for s in body.strategies],
...
async def save_history(body: ExperimentSave):
    exp = {
        ...
        "status": "completed", "tags": body.tags,
...
async def get_report(experiment_id: str):
    ...
    return {"summary": "Research report", "metrics": exp.get("results", {}), "charts": {}}
