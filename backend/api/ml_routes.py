...
AVAILABLE_FEATURES = [
    ...
    "volatility", "momentum_14", "rsi_14",
    "macd", "macd_signal", "macd_hist",
    "atr_14", "supertrend_upper", "supertrend_lower",
    "bb_upper", "bb_mid", "bb_lower",
    "ema_9", "ema_20", "ema_50", "sma_20", "sma_50",
    "vwap_distance", "volume_profile", "liquidity_score",
    "ai_score", "ai_confidence", "institutional_bias",
    "mtf_alignment", "market_regime", "sector_strength",
    "bos_count", "choch_count", "swing_strength",
]
...
async def list_features():
    return [{"name": f, "type": "continuous", "category": "auto"} for f in AVAILABLE_FEATURES]
...
async def train_model(body: TrainRequest):
    ...
    model = {
        ...
        "featureImportance": {f: round(random.random(), 4) for f in (body.features or AVAILABLE_FEATURES[:10])},
...
async def evaluate_model(body: EvaluateRequest):
    ...
    correct = sum(1 for p, a in zip(body.predictions, body.actuals) if (p >= 0.5) == (a == 1.0))
...
async def predict(body: PredictRequest):
    ...
    score = 0.5 + sum(v * random.random() for v in body.features.values()) / len(body.features) if body.features else 0.5
    return {
...
async def get_registry():
    ...
    champion = max(models, key=lambda m: m.get("metrics", {}).get("f1", 0)) if models else None
    return {
        "champion": champion,
        "challengers": [m for m in models if m.get("id") != (champion.get("id") if champion else None)],
...
async def detect_drift():
    return {
        ...
        "driftedMetrics": {"accuracy": random.random() > 0.7, "f1": random.random() > 0.7} if random.random() > 0.7 else {},
