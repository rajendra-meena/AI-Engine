...
class DetectedPattern:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"name": self.name, "direction": self.direction, "strength": self.strength, "time": self.candle_time}
...
class CandlestickPatternDetector:
    ...
    def update(self, candle: Candle) -> list[DetectedPattern]:
        ...
        if range_c > 0 and body_c < range_c * 0.1:
            new_patterns.append(DetectedPattern(name="doji", direction="neutral", strength="moderate", candle_time=candle.time))
        ...
        if range_c > 0 and body_c < range_c * 0.35:
            ...
            if lower > body_c * 2 and upper < body_c:
                new_patterns.append(DetectedPattern(name="hammer", direction="bullish", strength="strong", candle_time=candle.time))
            if upper > body_c * 2 and lower < body_c:
                new_patterns.append(DetectedPattern(name="shooting_star", direction="bearish", strength="strong", candle_time=candle.time))

        # Engulfing
        if range_p > 0:
            if c.close > c.open and c.open < p.low and c.close > p.high:
                new_patterns.append(DetectedPattern(name="bullish_engulfing", direction="bullish", strength="strong", candle_time=candle.time))
            if c.close < c.open and c.open > p.high and c.close < p.low:
                new_patterns.append(DetectedPattern(name="bearish_engulfing", direction="bearish", strength="strong", candle_time=candle.time))

        # Inside Bar
        if c.high <= p.high and c.low >= p.low:
            new_patterns.append(DetectedPattern(name="inside_bar", direction="neutral", strength="weak", candle_time=candle.time))

        # Outside Bar
        if c.high > p.high and c.low < p.low:
            direction = "bullish" if c.close > c.open else "bearish"
            new_patterns.append(DetectedPattern(name="outside_bar", direction=direction, strength="moderate", candle_time=candle.time))
        ...
        if body_c > 0 and range_c > 0 and body_c > range_c * 0.95:
            if upper_p < range_c * 0.02 or lower_p < range_c * 0.02:
                direction = "bullish" if c.close > c.open else "bearish"
                new_patterns.append(DetectedPattern(name="marubozu", direction=direction, strength="strong", candle_time=candle.time))
        ...
        if range_c > 0 and body_c < range_c * 0.4:
            ...
            if upper > body_c * 1.5 and lower > body_c * 1.5:
                new_patterns.append(DetectedPattern(name="spinning_top", direction="neutral", strength="weak", candle_time=candle.time))
